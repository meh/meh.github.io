...
"resources/config.xml")){miniLOL.config.core||(miniLOL.config.core={});if(!miniLOL.config.core.siteTitle)miniLOL.config.core.siteTitle="miniLOL #{version}".interpolate(miniLOL);if(!miniLOL.config.core.loadingMessage)miniLOL.config.core.loadingMessage="Loading...";if(miniLOL.config.core.homePage){if(miniLOL.config.core.homePage.charAt(0)!="#"&&!miniLOL.config.core.homePage.isURL())miniLOL.config.core.homePage="#"+miniLOL.config.core.homePage}else miniLOL.config.core.homePage="#home";if(!document.title)document.title=
miniLOL.config.core.siteTitle}});miniLOL.resource.set(new miniLOL.Resource("miniLOL.config",{initialize:function(){miniLOL.config=this.data},load:function(a){new Ajax.Request(a,{method:"get",asynchronous:false,onSuccess:function(c){if(!miniLOL.Document.check(c.responseXML,a)){c=miniLOL.Document.fix(c.responseXML).documentElement;var b=c.getAttribute("domain")||"core";miniLOL.config[b]=Object.extend(miniLOL.config[b]||{},Element.toObject(c))}},onFailure:function(c){miniLOL.error("miniLOL.config: Error while loading #{path} (#{status} - #{statusText})".interpolate({path:a,
...
function(){miniLOL.resource.set(new miniLOL.Resource("miniLOL.pages",{load:function(a,c){new Ajax.Request(a,{method:"get",asynchronous:false,onSuccess:function(b){miniLOL.Document.check(b.responseXML,a)||miniLOL.Document.fix(b.responseXML).xpath("//page").each(function(d){var f=d.getAttribute("id");delete miniLOL.pages.cache[f];miniLOL.pages.data[f]=d})},onFailure:function(b){c||miniLOL.error("miniLOL.pages: Error while loading #{path} (#{status} - #{statusText})".interpolate({path:a,status:b.status,
...
function(){miniLOL.content.set("Loading modules...");miniLOL.resource.set(new miniLOL.Resource("miniLOL.modules",{load:function(a,c){new Ajax.Request(a,{method:"get",asynchronous:false,onSuccess:function(b){if(!miniLOL.Document.check(b.responseXML,a)){miniLOL.module.path=b.responseXML.documentElement.getAttribute("path")||"modules";var d=miniLOL.Document.fix(b.responseXML).xpath("//module").filter(function(f){return f.getAttribute("name")});d.each(function(f,g){c&&miniLOL.content.set("Loading `#{name}`... [#{number}/#{total}]".interpolate({name:f.getAttribute("name"),
number:g+1,total:d.length}));miniLOL.module.load(f.getAttribute("name"))||miniLOL.error(true);if(miniLOL.error())throw $break;},this)}},onFailure:function(b){miniLOL.error("Error while loading modules.xml (#{status} - #{statusText})".interpolate(b),true)}});if(miniLOL.error())return false;return true},clear:function(){miniLOL.modules=this.data={}}}));miniLOL.resource.get("miniLOL.modules").load("resources/modules.xml",true)},function(){miniLOL.menu.enabled()&&miniLOL.menu.change("default")},function(){miniLOL.content.set("Checking dependencies...");
...
miniLOL.utils.get("#{path}/template.html".interpolate({path:g,theme:a}),{minified:true}))$(document.body).update(d);else{miniLOL.error("Could not load template.html.",true);return false}miniLOL.theme.information.styles.each(function(h){if(!miniLOL.theme.style.load(h,false,true)){miniLOL.error("Could not load `#{style}` style/".interpolate({style:h}),true);throw $break;}});if(miniLOL.error())return false;if(c&&miniLOL.initialized){miniLOL.menu.change(miniLOL.menu.current,true);miniLOL.go(location.href)}miniLOL.theme.tmp=
...
(l.firstChild?l.firstChild.nodeValue:m);var o=l.getAttribute("before")||d||"";l.removeAttribute("before");var r=l.getAttribute("after")||f||"";l.removeAttribute("after");var u=l.getAttribute("domain")||"";l.removeAttribute("domain");var s=l.getAttribute("arguments")||g;l.removeAttribute("arguments");var t=l.getAttribute("menu")||h;l.removeAttribute("menu");var p=l.getAttribute("title")||"";l.removeAttribute("title");var q=m.isURL(),v=l.getAttribute("class")||"";l.removeAttribute("class");var w=l.getAttribute("id")||
...
attributes:Object.fromAttributes(a.attributes)});return""}},load:function(a,c,b){miniLOL.content.set(miniLOL.config.core.loadingMessage);Event.fire(document,":page.load",{path:a,queries:c});if(b)document.title=(c.title||miniLOL.config.core.siteTitle).interpolate(Object.extend(Object.extend({},miniLOL.config.core),c));new Ajax.Request("data/#{path}?#{queries}".interpolate({path:a,queries:Object.toQueryString(c)}),{method:"get",onSuccess:function(d){c.type?miniLOL.content.set(miniLOL.resource.get("miniLOL.functions").render(c.type,
d.responseText,c)):miniLOL.content.set(d.responseText);Event.fire(document,":page.loaded",d);Event.fire(document,":go",b.getHashFragment())},onFailure:function(d){miniLOL.content.set("#{status} - #{statusText}".interpolate(d));Event.fire(document,":page.loaded",d)}})}},module:{create:function(a,c){if(!c){miniLOL.error("Like, do I know how this module is done?",true);return false}Event.fire(document,":module.create",{name:a,module:c});c.name=a;c.root="#{path}/#{module}".interpolate({path:miniLOL.module.path,
module:a});if(!Object.isArray(c.dependencies))c.dependencies=[];if(!c.type)c.type="active";if(!c.execute)c.execute=new Function;for(var b in c)if(Object.isFunction(c[b]))c[b]=c[b].bind(c);c.storage=new miniLOL.Storage("module."+a);if(c.initialize)try{if(c.initialize()===false){if(miniLOL.error())return false;throw Error("An error occurred while initializing the module.");}}catch(d){d.fileName="#{root}/#{path}/#{module}/main.js".interpolate({root:miniLOL.path,path:miniLOL.module.path,module:a});throw d;
}miniLOL.modules[a]=c;$A(c.aliases).each(function(f){miniLOL.modules[f]=c});document.fire(":module.created",c);return c},execute:function(a,c,b){if(!a){miniLOL.error("What module should be executed?");return false}if(!miniLOL.module.exists(a)){b&&miniLOL.error("The module `#{name}` is not loaded.".interpolate({name:a}));return false}if(b){miniLOL.content.set(miniLOL.config.core.loadingMessage);b={};Object.extend(b,miniLOL.config.core);Object.extend(b,c);document.title=(c.title||miniLOL.module.get(a).title||
miniLOL.config.core.siteTitle).interpolate(b)}c=Object.isArray(c)?c:[c];Event.fire(document,":module.execute",{name:a,arguments:c});var d;try{d=miniLOL.module.get(a).execute.apply(miniLOL.module.get(a),c)}catch(f){f.fileName="#{root}/#{path}/#{module}/main.js".interpolate({root:miniLOL.path,path:miniLOL.module.path,module:a});miniLOL.error("An error occurred while executing the module `#{name}`\n\n#{file} @ #{line}:\n#{error}".interpolate({name:a,file:f.fileName,line:f.lineNumber,error:f.toString()}),
true);return false}Event.fire(document,":module.executed",{name:a,arguments:c,result:d});if(Object.isUndefined(d))d=true;return d},load:function(a){Event.fire(document,":module.load",a);try{miniLOL.utils.require("#{path}/#{module}/main.js".interpolate({path:miniLOL.module.path,module:a}),{minified:true});if(miniLOL.error())return false;if(!miniLOL.module.exists(a))throw Error("Something went wrong while loading the module `#{name}`.".interpolate({name:a}));Event.fire(document,":module.loaded",a);
return true}catch(c){miniLOL.error("An error occurred while loading the module `#{name}`\n\n#{root}/#{file} @ #{line}:\n\n#{error}".interpolate({name:a,root:miniLOL.path,file:c.fileName,line:c.lineNumber,error:c.toString()}),true);return false}},get:function(a){return miniLOL.modules[a]},exists:function(a){return Boolean(miniLOL.module.get(a))},dependencies:{check:function(){for(var a in miniLOL.modules){var c=miniLOL.module.get(a).dependencies;if(c)for(var b=0;b<c.length;b++)if(!miniLOL.module.get(c[b]))throw{module:a,
require:c[b]};}return true},needs:function(a,c,b,d){miniLOL.module.get(a)?c.call(b||window):setTimeout(function(){miniLOL.module.dependencies.needs(a,c,b)},d||10)}}},go:function(a,c){if(!Object.isString(a))return false;if(a.isURL()){a=a.parseURL();if(a.host!=location.host||!a.path.startsWith(miniLOL.path))location.href=a.full;else a=a.full.replace(RegExp("^.*"+miniLOL.path),"")}else if(a.charAt(0)!="#")a="#"+a;var b=a.toQueryParams(),d=a.match(/#(([^=&]*)&|([^=&]*)$)/),f=false;b.menu&&miniLOL.menu.change(b.menu);
if(d){b.page=d[2]||d[3];if(b.page)f=miniLOL.page.get(b.page,b,a);else c||(f=miniLOL.go("#{page}&#{queries}".interpolate({page:miniLOL.config.core.homePage,queries:Object.toQueryString(b)}),true))}else if(b.module)f=miniLOL.module.execute(b.module,b,true);else if(b.page){d=b.page;delete b.page;miniLOL.page.load(d,b,a)}else f=miniLOL.go(miniLOL.config.core.homePage);f&&Event.fire(document,":go",a.getHashFragment());return f}};
