...
new Function;for(var b in c)if(Object.isFunction(c[b]))c[b]=c[b].bind(c);c.storage=new miniLOL.Storage("module."+a);if(c.initialize)try{if(c.initialize()===false){if(miniLOL.error())return false;throw Error("An error occurred while initializing the module.");}}catch(d){d.fileName="#{root}/#{path}/#{module}/main.js".interpolate({root:miniLOL.path,path:miniLOL.module.path,module:a});throw d;}miniLOL.modules[a]=c;$A(c.aliases).each(function(g){miniLOL.modules[g]=c});document.fire(":module.created",c);
...
{name:a,arguments:c});var d;try{d=miniLOL.module.get(a).execute.apply(miniLOL.module.get(a),c)}catch(g){g.fileName="#{root}/#{path}/#{module}/main.js".interpolate({root:miniLOL.path,path:miniLOL.module.path,module:a});miniLOL.error("An error occurred while executing the module `#{name}`\n\n#{file} @ #{line}:\n#{error}".interpolate({name:a,file:g.fileName,line:g.lineNumber,error:g.toString()}),true);return false}Event.fire(document,":module.executed",{name:a,arguments:c,result:d});if(Object.isUndefined(d))d=
...
c,b)},d||10)}}},go:function(a,c){if(!Object.isString(a))return false;if(a.isURL()){a=a.parseURL();if(a.host!=location.host||!a.path.startsWith(miniLOL.path))location.href=a.full;else a=a.full.replace(RegExp("^.*"+miniLOL.path),"")}else if(a.charAt(0)!="#")a="#"+a;var b=a.toQueryParams(),d=a.match(/#(([^=&]*)&|([^=&]*)$)/),g=false;b.menu&&miniLOL.menu.change(b.menu);if(d){b.page=d[2]||d[3];if(b.page)g=miniLOL.page.get(b.page,b,a);else c||(g=miniLOL.go("#{page}&#{queries}".interpolate({page:miniLOL.config.core.homePage,
queries:Object.toQueryString(b)}),true))}else if(b.module)g=miniLOL.module.execute(b.module,b,true);else if(b.page){d=b.page;delete b.page;miniLOL.page.load(d,b,a)}else g=miniLOL.go(miniLOL.config.core.homePage);g&&Event.fire(document,":go",a.getHashFragment());return g}};
